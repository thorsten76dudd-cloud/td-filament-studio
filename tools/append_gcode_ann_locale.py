"""Append gcode_ann.* keys to locale/de.py and locale/en.py."""
from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent / "creality_nfc" / "locale"

# (key, de, en) — ASCII-safe; umlauts as escapes
ENTRIES: list[tuple[str, str, str]] = [
    ...
    ("gcode_ann.filament_used", "Filamentverbrauch (Angabe im Kommentar)", "Filament usage (comment)"),
    ...
    ("gcode_ann.not_displayed", "Hinweis: Abschnitt in der Vorschau ausgelassen", "Note: section omitted in preview"),
    ("gcode_ann.bytes_middle", "Vorschau: Mittelteil der Datei fehlt", "Preview: middle of file omitted"),
    ...
    ("gcode_ann.g0_extrude", "Schnell bewegen und dabei Material extrudieren", "Rapid move while extruding"),
    ...
    ("gcode_ann.g1_extrude_axes", "Drucklinie \u2014 Material wird extrudiert ({axes})", "Print line \u2014 extruding ({axes})"),
    ...
    ("gcode_ann.g92", "Logische Position setzen (ohne physische Bewegung)", "Set logical position (no physical move)"),
    ("gcode_ann.g90", "Ab jetzt absolute Positionen (Millimeter vom Nullpunkt)", "Absolute coordinates from now on"),
    ("gcode_ann.g91", "Ab jetzt relative Positionen (Versatz zur letzten Position)", "Relative coordinates from now on"),
    ...
    ("gcode_ann.m83", "Extrusionsmenge ab jetzt relativ zur letzten Zeile", "Relative extrusion from now on"),
    ...
    ("gcode_ann.nonstandard", "Slicer- oder Drucker-Zeile (kein Standard-Bewegungsbefehl)", "Slicer/printer line (non-standard motion)"),
    ("gcode_ann.trail_sep", " \u2014 ", " \u2014 "),
]


def _append(path: Path, lang_idx: int) -> None:
    text = path.read_text(encoding="utf-8")
    if "gcode_ann.layer" in text:
        return
    lines = []
    for key, de, en in ENTRIES:
        val = de if lang_idx == 0 else en
...
def main() -> None:
    _append(ROOT / "de.py", 0)
    _append(ROOT / "en.py", 1)
    print("appended gcode_ann keys")
